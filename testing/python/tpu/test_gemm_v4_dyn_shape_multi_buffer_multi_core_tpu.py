...
def matmul_dyn_shape_pilelined_multicore(
    block_M: int = 128,
    block_N: int = 128,
    block_K: int = 32,
    num_stages: int = 3,
    ...
    def main(
        ...
        with T.Kernel(
                T.ceildiv(M, block_M),   # bx: M tiles per core
                T.ceildiv(N, block_N),   # by: N tiles
            ...
            bx_global = bc * M_tiles_per_core + bx
            with T.If(bx_global * block_M < M), T.Then():
                T.clear(C_local)

                for k in T.Pipelined(T.ceildiv(K, block_K), num_stages=num_stages):
                    T.copy(A[bx_global * block_M, k * block_K], A_local)
            ...
            T.copy(C_local, C[bx_global * block_M, by * block_N])

    return main

print("func_source:")
print(matmul_dyn_shape_pilelined_multicore.func_source)
print("signature:")
print(matmul_dyn_shape_pilelined_multicore.signature)

print("TIR:")
print(matmul_dyn_shape_pilelined_multicore.get_tir().script())

# kernel = matmul_dyn_shape_pilelined_multicore.compile()
kernel = matmul_dyn_shape_pilelined_multicore()
print("CUDA Source:")
print(kernel.get_kernel_source())
print("\nDynamic Shape Pipelined Multi Core GEMM+ReLU compilation for TPU target succeeded.")
